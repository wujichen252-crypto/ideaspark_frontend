<template>
  <div class="workbench-container">
      ...
      <n-layout-content class="workbench-content">
        ...
        <div v-else class="content-placeholder">
          <n-empty description="此区域暂无内容">
            <template #extra>
              <div class="debug-info">当前选中: {{ activeKey }}</div>
            </template>
          </n-empty>
        </div>
      </n-layout-content>
    ...
    <n-modal v-model:show="showRenameModal">
      ...
      >
        <div class="modal-content">
          <n-input v-model:value="renameTeamName" placeholder="输入新的团队名称" size="large" />
        </div>
        <template #footer>
          <div class="modal-footer">
...
.workbench-container {
  height: 100vh;
  /* padding-top: 56px; 移除外部 padding，让内容可以滚动到 Header 下方 */
  display: flex;
...
.workbench-sider {
  ...
  padding-top: 56px; /* 内部 padding，避开 Fixed Header */
  ...
  .sider-logo {
    ...
    align-items: center;
    padding: 0 24px;
    display: none; /* Logo 已经在 AppHeader 中显示，这里隐藏或根据需求调整 */
...
.workbench-content {
  ...
  padding: 0;
  padding-top: 56px; /* 内部 padding，让内容初始位置正确，滚动时穿过 Header */
}
